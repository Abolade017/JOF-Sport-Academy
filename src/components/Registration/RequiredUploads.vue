...
const birthCertificate = (e: Event) => {
  const files = (e.target as HTMLInputElement).files
  if (files && files.length > 0) {
    store.requiredUploads.uploads_birth_certificate = files[0]
  }
}

const consentLetter = (e: Event) => {
  const files = (e.target as HTMLInputElement).files
  if (files && files.length > 0) {
    store.requiredUploads.uploads_consent_letter = files[0]
  }
}

const otherDocuments = (e: Event) => {
  const files = (e.target as HTMLInputElement).files
  if (files && files.length > 0) {
    store.requiredUploads.uploads_other_document = files[0]
  }
}
const consent1 = computed({
  get: () => store.requiredUploads.consent_to_training,
  set: (val: boolean) => (store.requiredUploads.consent_to_training = val),
})
const consent2 = computed({
  get: () => store.requiredUploads.confirm_information,
  set: (val: boolean) => (store.requiredUploads.confirm_information = val),
})
const consent3 = computed({
  get: () => store.requiredUploads.agree_terms,
  set: (val: boolean) => (store.requiredUploads.agree_terms = val),
})
const hasSubmitted = ref(false)
const allConsentsChecked = computed(() => {
  return consent1.value && consent2.value && consent3.value
})
const uploadsValid = computed(() => birthCertificate && consentLetter && otherDocuments)
  ...
  <div class="mt-8 md:mt-16 font-zalando">
    <RegistrationForm title=" Required uploads">
      <div class="flex flex-col gap-5 md:my-6 my-4">
        ...
        <p v-if="hasSubmitted && !birthCertificate" class="text-sm text-red-500">
