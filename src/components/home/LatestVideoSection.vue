...
const currentVideo = computed(() => {
  return videos.slice(startIndex.value, startIndex.value + ITEMS_PER_PAGE)
})
// console.log(currentFixture)
const next = () => {
  if (startIndex.value + ITEMS_PER_PAGE < videos.length) {
...
<template>
  <div class="w-full md:max-w-[1216px] mx-auto bg-[#F5F6F7] font-zalando">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 pt-8 md:mt-10 px-6 md:px-16 pb-8">
      <div v-for="(video, index) in currentVideo" :key="startIndex + index" class="relative w-full">
